...
static int da7218_stream_enable(struct snd_soc_component *component)
{
        printk(" SMET inside  %s %d \n",__FUNCTION__,__LINE__);
	printk("Execute da7218_stream_enable()\n");
	snd_soc_component_write(component, DA7218_SYSTEM_ACTIVE, 0x01);			//Enable ACTIVE mode
	snd_soc_component_write(component, DA7218_CIF_CTRL, 0x01);			//Enable repeat mode register access
	snd_soc_component_write(component, DA7218_REFERENCES, 0x08);			//Enable Master BIAS and VMID
	snd_soc_component_write(component, DA7218_GAIN_RAMP_CTRL, 0x01);		//Set ramp rate to nominal

	snd_soc_component_write(component, DA7218_PLL_CTRL, 0x02);			//PLL Disabled, PLL INDIV= 10-20MHz
	snd_soc_component_write(component, DA7218_IO_CTRL, 0x01);			//IO Level 1.5-2.5V
	snd_soc_component_write(component, DA7218_IO_CTRL, 0x80);			//Digital LDO Enabled, 1.05V
	snd_soc_component_write(component, DA7218_PLL_FRAC_TOP, 0x00);			//PLL FRAC_TOP=00
	snd_soc_component_write(component, DA7218_PLL_FRAC_BOT, 0x00);			//PLL FRAC_BOT=00
	snd_soc_component_write(component, DA7218_PLL_INTEGER, 0x20);			//PLL Set-up for 12.288MHz MCLK
	snd_soc_component_write(component, DA7218_SR, 0xBB);				//48kHz sample rate for ADC and DAC
	snd_soc_component_write(component, DA7218_PLL_CTRL, 0x82);			//PLL Enabled, SRM Mode, PLL INDIV= 10-20MHz
	snd_soc_component_write(component, DA7218_DAI_CTRL, 0xA0);			//DAI Enabled, DAI Channel 1L and 1R enabled, 16bit, I2S
	snd_soc_component_write(component, DA7218_DAI_CLK_MODE, 0x01);			//Slave Mode DAI
	snd_soc_component_write(component, DA7218_DAI_OFFSET_LOWER, 0x00);		//No Offset on DAI (lower)
	snd_soc_component_write(component, DA7218_DAI_OFFSET_UPPER, 0x00);		//No Offset on DAI (upper)

	snd_soc_component_write(component, DA7218_IO_CTRL, 0x01);			//IO Level 1.5-2.5V
	snd_soc_component_write(component, DA7218_LDO_CTRL, 0x80);			//Digital LDO Enabled, 1.05V
	snd_soc_component_write(component, DA7218_DAI_OFFSET_LOWER, 0x00);		//No Offset on DAI (lower)
	snd_soc_component_write(component, DA7218_DAI_OFFSET_UPPER, 0x00);		//No Offset on DAI (upper)

	snd_soc_component_write(component, DA7218_CP_CTRL, 0xF0);			//CP Enabled, Signal Magnitude tracking mode
	snd_soc_component_write(component, DA7218_CP_DELAY, 0x35);			//CP Delay 64ms
	snd_soc_component_write(component, DA7218_CP_VOL_THRESHOLD1, 0x36);		//CP Volume threshold
	printk("da7218_stream_enable() Finished\n");
	
	return 0;
}

static int da7218_playback(struct snd_soc_component *component)
{
        printk(" SMET inside  %s %d \n",__FUNCTION__,__LINE__);
	printk("Execute da7218_playback()\n");
	snd_soc_component_write(component, DA7218_DROUTING_OUTFILT_1L, 0x20);			//OUTFILT_1L_SOURCE = DAI INPUT L
	snd_soc_component_write(component, DA7218_DROUTING_OUTFILT_1R, 0x40);			//OUTFILT_1R_SOURCE = DAI INPUT R
	
	snd_soc_component_write(component, DA7218_DMIX_OUTFILT_1L_INDAI_1L_GAIN, 0x1C);		//DAI INPUT L GAIN = 0dB
	snd_soc_component_write(component, DA7218_DMIX_OUTFILT_1R_INDAI_1R_GAIN, 0x1C);		//DAI INPUT L GAIN = 0dB

	snd_soc_component_write(component, DA7218_OUT_1L_GAIN, 0x6F);				//OUT_1L_DIGITAL_GAIN GAIN = 0dB
	snd_soc_component_write(component, DA7218_OUT_1R_GAIN, 0x6F);				//OUT_1R_DIGITAL_GAIN GAIN = 0dB

	snd_soc_component_write(component, DA7218_MIXOUT_L_CTRL, 0x01);				//MIXOUT_L Enable
	snd_soc_component_write(component, DA7218_MIXOUT_R_CTRL, 0x01);				//MIXOUT_R Enable

	snd_soc_component_write(component, DA7218_MIXOUT_L_GAIN, 0x03);				//MIXOUT_L GAIN = 0dB
	snd_soc_component_write(component, DA7218_MIXOUT_R_GAIN, 0x03);				//MIXOUT_R GAIN = 0dB

	snd_soc_component_write(component, DA7218_OUT_1L_FILTER_CTRL, 0x80);			//Enable OUT_1L_FILTER
	snd_soc_component_write(component, DA7218_OUT_1R_FILTER_CTRL, 0x80);			//Enable OUT_1R_FILTER

	snd_soc_component_write(component, DA7218_MIXOUT_L_CTRL, 0x80);				//MIXOUT_L Enable
	snd_soc_component_write(component, DA7218_MIXOUT_R_CTRL, 0x80);				//MIXOUT_R Enable

	snd_soc_component_write(component, DA7218_HP_L_CTRL, 0xE8);				//HP_L Enable, Muted, Ramped
	snd_soc_component_write(component, DA7218_HP_R_CTRL, 0xE8);				//HP_R Enable, Muted, Ramped

	snd_soc_component_write(component, DA7218_DGS_LEVELS, 0x00);				//DGS SIGNAL and ANTICLIP level to 0dB
	snd_soc_component_write(component, DA7218_DGS_TRIGGER, 0x27);				//Enable DGS

	snd_soc_component_write(component, DA7218_HP_L_CTRL, 0xA8);				//HP_L Enable, Un-muted, Ramped
	snd_soc_component_write(component, DA7218_HP_R_CTRL, 0xA8);				//HP_R Enable, Un-muted, Ramped
	printk("da7218_playback() Finished\n");
	
	return 0;
}

static int da7218_capture(struct snd_soc_component *component)
{
        printk(" SMET inside  %s %d \n",__FUNCTION__,__LINE__);
	printk("Execute da7218_capture()\n");
	snd_soc_component_write(component, DA7218_MIC_1_CTRL, 0x80);				//MIC1 Amp Enabled
	snd_soc_component_write(component, DA7218_MIC_2_CTRL, 0x80);				//MIC2 Amp Enabled

	snd_soc_component_write(component, DA7218_MIXIN_1_CTRL, 0xA8);				//MIXIN_1 Enabled
	snd_soc_component_write(component, DA7218_MIXIN_2_CTRL, 0xA8);				//MIXIN_2 Enabled

	snd_soc_component_write(component, DA7218_IN_2_HPF_FILTER_CTRL, 0x00);			//Disable IN_1 HPF
	snd_soc_component_write(component, DA7218_IN_1_HPF_FILTER_CTRL, 0x00);			//Disable IN_2 HPF

	snd_soc_component_write(component, DA7218_IN_1L_FILTER_CTRL, 0xA0);			//INFILT_1_L Enabled
	snd_soc_component_write(component, DA7218_IN_2L_FILTER_CTRL, 0xA0);			//INFILT_2_L Enabled

	snd_soc_component_write(component, DA7218_DROUTING_OUTDAI_1L, 0x1);			//INFILT_1_L to DAI_1_L
	snd_soc_component_write(component, DA7218_DROUTING_OUTDAI_1R, 0x4);			//INFILT_2_L to DAI_1_R

	snd_soc_component_write(component, 0x11, 0x8B);						//UNLOCK
	snd_soc_component_write(component, DA7218_ADC_1_CTRL, 0x03);				//ADC_1_CTRL
	snd_soc_component_write(component, DA7218_ADC_2_CTRL, 0x03);				//ADC_2_CTRL
	snd_soc_component_write(component, DA7218_REFERENCES, 0x09);				//References
	snd_soc_component_write(component, 0x11, 0x00);						//UNLOCK
	printk("da7218_capture() Finished\n");
			    ...
			    struct snd_soc_dai *dai)
{
	...
	switch (substream->stream)
	{
		case SNDRV_PCM_STREAM_PLAYBACK:
			da7218_stream_enable(component);
			da7218_playback(component);
			break;
		case SNDRV_PCM_STREAM_CAPTURE:
			da7218_stream_enable(component);
			da7218_capture(component);
			break;
	}
...
static int da7218_probe(struct snd_soc_component *component)
{
	...
	//snd_soc_component_write(component, DA7218_DROUTING_OUTFILT_1L, 32);
	snd_soc_component_write(component, DA7218_DROUTING_OUTFILT_1R, 0);
	//snd_soc_component_write(component, DA7218_DROUTING_OUTFILT_1R, 64);
